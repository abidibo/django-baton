...
class TestBatonIndexMobile(TestCase):
    ...
    def test_navbar(self):
        ...
        # user dropdown
        user_dropdown_el = self.driver.find_element(
            By.CSS_SELECTOR, "#user-tools .dropdown-toggle"
        )
        ...
        # user dropdown menu
        user_dropdown_menu = self.driver.find_elements(
            By.CSS_SELECTOR, "#user-tools .dropdown-menu a"
        )
        self.assertEqual(len(user_dropdown_menu), 5)
        self.assertEqual(user_dropdown_menu[0].get_attribute("innerHTML"), "View site")
        self.assertEqual(
            user_dropdown_menu[1].get_attribute("innerHTML"), "Documentation"
        )
        self.assertEqual(
            user_dropdown_menu[2].get_attribute("innerHTML"), "Change password"
        )
        self.assertEqual(user_dropdown_menu[3].get_attribute("innerHTML"), "Log out")
        self.assertEqual(
            user_dropdown_menu[4]
    ...
    def test_footer(self):
        ...
        # copyright
        self.assertEqual(
            links[1].get_attribute("innerText").strip(), "Copyright © 2026 Otto srl"
        )
        # powered by
        self.assertEqual(
